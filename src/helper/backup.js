...
export async function buildBackup() {
  const [library, settings, progress] = await Promise.all([
    getLibrary(),
    getSettings(),
    getAllProgress(),
  ])
...
export function readBackup(text) {
  ...
  if (!movies.length && !settings && !Object.keys(progress).length) {
    throw new Error('That file holds nothing to import.')
  }

  return { movies, settings, progress }
}
 ...
 */
export async function applyBackup({ movies, settings, progress }) {
  ...
  return { added, updated: movies.length - added, positions: Object.keys(progress).length }
}
